...
export function SettingsRow({ children, description, icon, label }: SettingsRowProps) {
  return (
    <div className="flex w-full min-w-0 flex-wrap items-center gap-3 rounded-[18px] border border-white/[0.06] bg-white/[0.03] p-3">
      ...
      {children && <div className="ml-auto min-w-0 shrink-0">{children}</div>}
    </div>
  );
}
